module.exports = function (schema, option) {
    ...
    const styleFilter = (style) => {
        ...
        if (style) {
            _.forEach(style, (value, key) => {
                if (flexKeys.includes(key)) {
                    ...
                    console.log('out style', newStyle);
                } else {
                    const str = `${_.kebabCase(key)}: ${value}`;
        ...
        console.log("class", extraClasses);
        return {
            extraClasses,
            newStyle
        };
    ...
    const generateRender = (schema) => {
        ...
        const {
            extraClasses,
            newStyle
        } = styleFilter(_.get(schema.props, 'style'));
        const classList = [];
        ...
        switch (type) {
            ...
            case 'component':
                if (schema.children && schema.children.length) {
                    xml = `<view${classString}${styleString}${props}>${transform(schema.children)}</view>`;
                } else {
